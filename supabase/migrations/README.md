...
| `0047_event_checkin.sql` | `event_registrations.checked_in_at` — QR-based self check-in. Organizers/admin display one QR per event (`/events/[slug]/check-in`); a registrant scans it, signs in, and self-confirms attendance. No new token or RLS policy — reuses the existing owner-write policy. Attendance-only, no reward flow. |
