// Creates test users for marketplace flow testing:
//   - a test parent (to post jobs)
//   - a test caregiver (to apply for jobs)
// Idempotent. Password from env TEST_PASSWORD (default below). Run:
//   node --env-file=.env.local scripts/make-marketplace-test-users.mjs
import pg from "pg";
...
const parentEmail = "marketplace-test-parent@raisingclub-test.dev";
const parentId = await createUser(parentEmail);
await a.query("update profiles set role='parent', first_name='John', last_name='Smith', preferred_name='John', registered_at=now(), onboarding_completed_at=now() where id=$1", [parentId]);

const cgEmail = "marketplace-test-caregiver@raisingclub-test.dev";
const cgId = await createUser(cgEmail);
await a.query("update profiles set role='caregiver', first_name='Emily', last_name='Brown', preferred_name='Emily', registered_at=now(), onboarding_completed_at=now() where id=$1", [cgId]);
await a.query("insert into caregiver_profiles (user_id, headline, about, is_published) values ($1,'Experienced Educator','I specialize in early childhood development.',true) on conflict (user_id) do update set is_published=true", [cgId]);
...
console.log("Marketplace test users ready (password:", pw + ")");
